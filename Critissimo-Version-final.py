...
app = ctk.CTk()
app.title("Critissimo v1.6 Version Final")
...
main_container = ctk.CTkFrame(app, fg_color="#e8f5e9")
main_container.pack(fill="both", expand=True, padx=30, pady=30)

ctk.CTkLabel(
    main_container,
    text="CRITISSIMO v1.6 Version Final",
    font=("Arial", 36, "bold"),
    text_color="#00C853"
).pack(pady=10)
...
tab_monitoreo = tabview.add("Monitoreo Interno Rack")
...
frame_mon = tab_monitoreo
# === REGISTRO DE RESIDUOS Segun la Normativa Legal D.S. 148/2003 ===
scroll_reg = ctk.CTkScrollableFrame(tab_registro, fg_color="transparent")
scroll_reg.pack(fill="both", expand=True, padx=20, pady=20)
...
for i, (label_text, placeholder_text) in enumerate(campos):
    ctk.CTkLabel(scroll_reg, text=f"{i+1}. {label_text}", font=("Arial", 14, "bold"),
...
info_label = ctk.CTkLabel(
    scroll_reg,
    text=" Clase I: Muy Peligroso(ej: residuos infecciosos, mercurio)\n "
...
scroll_hist = ctk.CTkScrollableFrame(tab_historial, fg_color="transparent")
scroll_hist.pack(fill="both", expand=True, padx=20, pady=20)
...
# Conteo de críticos
label_criticos = ctk.CTkLabel(scroll_hist, text="", font=("Arial", 18, "bold"))
...
def cargar_historial():
    if tabview.get() == "cargar_historial":
        cargar_historial()
...
# === BOTÓN ACTUALIZAR ===
frame_hist_btn = ctk.CTkFrame(scroll_hist)
...
def mostrar_matriz():
    ...
    # DATOS DE LA MATRIZ
    filas = ["Improbable", "Posible", "Ocasional",
...
# == ALERTAS PREDICTIVAS INTERNAS ===
frame_alertas = ctk.CTkFrame(frame_mon, fg_color="#FFFFFF", corner_radius=15)
frame_alertas.pack(fill="x", pady=20, padx=80)

ctk.CTkLabel(
    frame_alertas,
    text="ALERTAS PREDICTIVAS INTERNAS:",
    font=("Arial", 20, "bold"),
    text_color="#FF1744"
).pack(pady=10)
...
label_alertas = ctk.CTkLabel(
    frame_alertas,
    text="Todo normal en este instante",
    font=("Arial", 16),
    text_color="#00C583",
    justify="left"
)
label_alertas.pack(pady=10, padx=20)

# Variables Simples
alertas_cont = 0
vibración_promedio = 0.0
turbidez_ultimas = []


# === MONITOREO INTERNO RACK ===
ctk.CTkLabel(
    frame_mon,
    text="MONITOREO INTERNO DE RACK\nDatos en tiempo real desde sensores IoT",
    font=("Arial", 28, "bold"),
    text_color="#00C853",
).pack(pady=(0, 40))
...
for nombre in sensores_lista:
    frame_sensor = ctk.CTkFrame(
        frame_mon, fg_color="#FFFFFF", corner_radius=15)
    frame_sensor.pack(fill="x", pady=10, padx=80)
    ...
    label_valor.pack(side="left", expand=True, pady=20)

    labels_sensores[nombre] = label_valor

 # Estado de conexión
label_ultima = ctk.CTkLabel(
    frame_mon,
    text="Verifica la conexión",
    font=("Arial", 16),
    text_color="#FF1744"
)
label_ultima.pack(pady=30)


ESP_IP = "10.114.251.221"


def obtener_datos_esp32():
    try:
        response = requests.get(f"http://{ESP_IP}/json", timeout=3)
...
def actualizar_monitoreo():
    ...
    if datos:
        ...
        for sensor in datos:
            nombre = sensor.get("name", "Desconocido")
            valor = sensor.get("value", "N/A")
            unidad = sensor.get("unit_of_measurement", "")
            ...
            if "Temperatura" in nombre:
                temp = valor
            elif "Vibración X" in nombre:
                vib_x = valor
            elif "Vibración Y" in nombre:
                vib_y = valor
            elif "Vibración Z" in nombre:
                vib_z = valor
            elif "Turbidez NTU" in nombre:
                turbidez = valor

          # Alertas visuales
            if "Temperatura" in nombre and valor > 40:
                color = "#FF1744"
                alertas_text.append(
                    "¡SOBRECALENTAMIENTO CRÍTICO! Temperatura > 40°C")
                es_critico = True
            elif "Temperatura" in nombre and valor > 35:
                color = "#FF6B00"
                alertas_text.append("Advertencia: Temperatura alta > 35°C")
            elif "Turbidez NTU" in nombre and valor > 1000:
                color = "#FF1744"
                alertas_text.append(
                    "LUBRICANTES/ACEITES CONTAMINADOS! Turbidez > 1000 NTU")
                es_critico = True
            elif "Vibración" in nombre and valor > 0.5:
                color = "#FF1744"
                alertas_text.append(
                    f"VIBRACIÓN ANORMAL EN {nombre}! Posible falla")

            if nombre in labels_sensores:
                labels_sensores[nombre].configure(text=texto, text_color=color)

        # Contabilización
        vibración_promedio = (vib_x + vib_y + vib_z) / 3
        turbidez_ultimas.append(turbidez)
        if len(turbidez_ultimas) > 5:
            turbidez_ultimas.pop(0)
        turbidez_prom = sum(turbidez_ultimas) / \
            len(turbidez_ultimas) if turbidez_ultimas else 0

        if alertas_text:
            alertas_cont += 1
            label_alertas.configure(
                text="\n".join(
                    alertas_text) + f"\n\nContador alertas: {alertas_cont}\nVibración_promedio: {vibración_promedio:.2f} m/s²\nTurbidez promedio (lubricantes): {turbidez_prom:.2f} NTU",
                text_color="#FF1744"
            )
            ...
            if es_critico:
                ...
                label_alertas.configure(
                    text="Todo bien\nContador alertas: {alertas:cont}\nVibración promedio: {vibración_promedio:.2f} m/s²\nTurbidez promedio (lubricantes): {turbidez_prom:.2f} NTU",
                    text_color="#00C853"
                )

            label_ultima.configure(
                text=f"Conecrado - Ultima vez: {datetime.now().strftime('%H:%M:%S')}",
            )
        else:
            label_alertas.configure(
                text="Sin conexión - No se pueden generar alertas", text_color="#FF1744")
            label_ultima.configure(
                text="Sin conexión - Verifica la conexión y el ESP32 encencido", text_color="#FF1744")

        app.after(5000, actualizar_monitoreo)

# == Detectar Cambios de pestañas ==


def check_tab():
    current_tab = tabview.get()
    if current_tab == "Monitoreo Interno Rack":
